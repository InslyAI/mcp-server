...
export function registerGetClaimFeatureTool(server: McpServer) {
  server.tool(
    "site_service_claim_features_feature_get",
    "Get specific claim feature configuration",
    {
    ...
    async ({ bearerToken, tenantId, tenantTag, schemaName, featureName }) => {
      try {
        ...
        const data = await client.get(`/api/v1/sites/claim-features/${tenantTag}/${schemaName}/${featureName}`);
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({ 
              success: true, 
              data,
              message: `Retrieved claim feature '${featureName}' for schema '${schemaName}' in tenant '${tenantTag}'`
            }, null, 2)
          }]
        };
      } catch (error) {
        return {
          content: [{
            ...
            text: JSON.stringify({ 
              ...
              details: `Failed to get claim feature '${featureName}' for schema '${schemaName}' in tenant '${tenantTag}'`
            }, null, 2)
          }]
        };
      }
