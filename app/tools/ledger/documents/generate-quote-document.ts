  ...
  server.tool(
    ...
    "Generate the main quote document or a specific document type for a quote. Returns binary documents in PDF/Word format for delivery and compliance content",
    {
      ...
      quoteId: z
        ...
        .int()
        .positive()
        .describe("Unique identifier of the quote"),
      documentType: z
        .string()
        .optional()
        .describe(
          "Specific document type to generate (e.g., 'wording'). If not provided, generates main document",
        ),
    },
    async ({ bearerToken, tenantId, quoteId, documentType }) => {
      try {
        ...
        const url = `https://${tenantId}.app.devbox.insly.training${endpoint}`;
                ...
                text: JSON.stringify(
                  {
                    ...
                    usage:
                      "Quote document generated successfully. The documentBase64 field contains the binary document encoded in base64.",
                    relatedTools: {
                      policy:
                        "Use ledger_generate_policy_document for policy documents",
                      upload:
                        "Use ledger_upload_quote_files to add additional documents",
                      issue:
                        "Use ledger_issue_quote to convert quote to policy",
                    },
                  },
                  null,
                  2,
