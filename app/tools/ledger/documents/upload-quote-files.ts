...
export function registerUploadQuoteFilesTools(server: McpServer) {
  server.tool(
    ...
    async ({ bearerToken, tenantId, quoteId, files }) => {
      try {
        const url = `https://${tenantId}.app.devbox.insly.training/api/v1/ledger/sales/policies/${quoteId}/files`;
              ...
              text: JSON.stringify(
                {
                  ...
                  relatedTools: {
                    generate:
                      "Use ledger_generate_quote_document to create quote documents",
