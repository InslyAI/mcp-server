...
export function registerLedgerListBinderNamesTool(server: McpServer) {
  server.tool(
    "ledger_sales_binders_names_list",
    "Get list of binder names and IDs for dropdown/selection purposes. Returns simplified binder data optimized for UI components.",
    {
      ...
      filterProduct: z
        .string()
        .optional()
        .describe("Filter binders by product"),
      filterStatus: z
        .array(z.string())
        .optional()
        .describe("Filter by binder status (e.g., ['active'])"),
      filterBroker: z.string().optional().describe("Filter binders by broker"),
      selectedBinderId: z
        .string()
        .optional()
        .describe("Binder with this ID will be added to response"),
    },
    async ({
      bearerToken,
      tenantId,
              ...
              text: JSON.stringify(
                {
                  ...
                  usage:
                    "Use the 'value' field as binder ID and 'label' field for display names",
                  relatedTools: {
                    fullDetails:
                      "Use ledger_list_binders for complete binder information",
                    getSpecific:
                      "Use ledger_get_binder with a specific binder ID",
                  },
                },
                null,
                2,
              ...
              text: JSON.stringify(
                {
                  ...
                  troubleshooting: {
                    authentication:
                      "Verify bearerToken is valid (use identifier_login to get new token)",
                    tenant:
                      "Ensure tenantId is correct (should be 'accelerate')",
                    network:
                      "Check network access to https://accelerate.app.devbox.insly.training",
                    permissions: "Verify user has access to binder data",
                    filters:
                      "Check that filter values are valid for your tenant",
                    headers:
                      "Ensure both Authorization and X-Tenant-ID headers are sent",
                  },
                  testCredentials: {
                    getToken:
                      "identifier_login({ username: 'rasim.mehtijev@insly.com', password: '[password]', tenantTag: 'accelerate' })",
                    usageExample: "ledger_sales_binders_names_list",
                  },
                },
                null,
                2,
