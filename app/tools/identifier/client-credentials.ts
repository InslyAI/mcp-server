...
export function registerIdentifierClientCredentialsTool(server: McpServer) {
  server.tool(
    ...
    async ({ clientId, clientSecret, tenantTag, scope }) => {
      try {
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                tenant: tenantTag,
                scope: scope || "default",
                message: "Client credentials authentication successful! Use the access_token as bearerToken for Ledger API calls.",
                usage: {
